...
const ACTIONS = [
  { label: "Freeze",   toast: "Card frozen! ❄",       icon: <><path d="M12 2v20M2 12h20M4.93 4.93l14.14 14.14M19.07 4.93L4.93 19.07" /></> },
  { label: "Details",  toast: "Card details shown 👁",  icon: <><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></> },
      ...
      <div className="flex-1 bg-[#f2f3f5] rounded-t-3xl -mt-3 overflow-y-auto pb-4">
          ...
          <div className={`rounded-[20px] p-5 relative overflow-hidden transition-opacity ${frozen ? "opacity-60" : ""}`} style={{ background: "linear-gradient(135deg,#1a7a52,#19a46a,#25c57a)" }}>
            ...
            {frozen && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/30 rounded-[20px]">
                <div className="text-center"><span className="text-4xl">❄</span><p className="text-white font-bold text-sm mt-1">Card Frozen</p></div>
              </div>
            )}
            <div className="w-8 h-6 rounded bg-yellow-400 mb-4" />
            <p className="text-white text-[15px] font-semibold tracking-[3px] mb-3">8012  ••••  ••••  3456</p>
        ...
        <div className="grid grid-cols-3 gap-2.5 px-3.5 mb-4">
          {ACTIONS.map(({ label, toast, icon }) => (
            <button key={label} onClick={() => { if (label === "Freeze") setFrozen(f => !f); showToast(label === "Freeze" ? (frozen ? "Card unfrozen! ✅" : "Card frozen! ❄") : toast) }} className="bg-white rounded-xl py-3 flex flex-col items-center gap-1.5">
        ...
        <div className="px-3.5">
          ...
          <div className="bg-white rounded-2xl overflow-hidden">
            {[{ icon: "", name: "Online Shopping", sub: "Jumia • Card Payment", amt: "-₦12,500" }, { icon: "💳", name: "Food Delivery", sub: "Chowdeck • Card Payment", amt: "-₦4,800" }].map(({ icon, name, sub, amt }) => (
